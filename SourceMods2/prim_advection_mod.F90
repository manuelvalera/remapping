...
!JMD!$OMP BARRIER
  ...
  subroutine vertical_remap(hybrid,elem,fvm,hvcoord,dt,np1,np1_qdp,n_fvm,nets,nete)
    ...
    use hybvcoord_mod, only          : hvcoord_t
    use vertremap_mod,          only : remap1, remap1_nofilter,remap2
    ...
    type (hvcoord_t) :: hvcoord
    real (kind=r8)   :: dt
    integer          :: ie,i,j,k,np1,nets,nete,np1_qdp,q, m_cnst,rmp_kind
...
!    call initialize_remapping(CSP,'PQM_IH6IH5',.true.,.false.,.false.,.true.)  
!    call initialize_remapping(CSP,'PPM_IH4',.true.,.false.,.false.,.true.)  
!    call initialize_remapping(CSP,'PCM',.true.,.false.,.false.,.true.)  
    ...
    do ie=nets,nete
      ...
      call remap2(elem(ie)%state%Qdp(:,:,:,1:qsize,np1_qdp),&
                np,1,qsize,qsize,dp_star_dry,dp_dry,rmp_kind,1)
      ...
      !      
      if (lcp_moist) then
        call remap2(internal_energy_star,np,1,1,1,dp_star_dry,dp_dry,rmp_kind,1)
!        call remap1(internal_energy_star,np,1,1,1,dp_star_dry,dp_dry)
        ...
        call remap2(internal_energy_star,np,1,1,1,dp_star_moist,dp_moist,rmp_kind,1)
!        call remap1(internal_energy_star,np,1,1,1,dp_star_moist,dp_moist)
      ...
      call remap2(ttmp,np,1,2,2,dp_star_moist,dp_moist,rmp_kind,1)
