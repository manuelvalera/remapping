...
module prim_driver_mod
  ...
  public :: smooth_topo_datasets, prim_set_dry_mass
                   ...
                   do i=1,np
                      do j=1,np
                         dp = ( hvcoord%hyai(k+1) - hvcoord%hyai(k) )*hvcoord%ps0 + &
                              ( hvcoord%hybi(k+1) - hvcoord%hybi(k) )*elem(ie)%state%psdry(i,j,t)
...
!
    ...
    use hack_vert_rmp,          only: lhack_vert_rmp
...
!    type (hybrid_t) :: vybrid
    ...
    do ie=nets,nete
      ...
      do k=1,nlev    !  Loop inversion (AAM)
        ...
        dp_np1(:,:) = ( hvcoord%hyai(k+1) - hvcoord%hyai(k) )*hvcoord%ps0 + &
             ( hvcoord%hybi(k+1) - hvcoord%hybi(k) )*elem(ie)%state%psdry(:,:,tl%np1)
    ...
    if (nsubstep==nsplit) &
         call compute_omega(hybrid,tl%np1,elem,deriv,nets,nete,dt,hvcoord)   
