 ...
 !$omp parallel do num_threads (vert_num_threads) private(dp)
    ...
    if (tstep_type==1) then
      ...
      call compute_and_apply_rhs(np1,n0,n0,dt/2,elem,hvcoord,hybrid,&
           deriv,nets,nete,eta_ave_w/3,inv_cp_full,qn0)
      ! u2 = u1 + dt/2 RHS(u1)
      ...
      call compute_and_apply_rhs(np1,n0,n0,dt/3,elem,hvcoord,hybrid,&
           deriv,nets,nete,0.0_r8,inv_cp_full,qn0)
      ! u2 = u0 + dt/2 RHS(u1)
      ...
      call compute_and_apply_rhs(np1,n0,np1,dt,elem,hvcoord,hybrid,&
           deriv,nets,nete,eta_ave_w,inv_cp_full,qn0)
      ...
      call compute_and_apply_rhs(nm1,n0,n0,dt/5,elem,hvcoord,hybrid,&
           deriv,nets,nete,eta_ave_w/4,inv_cp_full,qn0)
      !
      ...
      call compute_and_apply_rhs(np1,nm1,np1,3*dt/4,elem,hvcoord,hybrid,&
           deriv,nets,nete,3*eta_ave_w/4,inv_cp_full,qn0)
          ...
          do k=1,nlev
            do j=1,np
              do i=1,np
                ...
                !
                v1 = dt_local_tracer*elem(ie)%derived%FQ(i,j,k,q,1)
  ...
  subroutine advance_hypervis_dp(edge3,elem,hybrid,deriv,nt,qn0,nets,nete,dt2,eta_ave_w,inv_cp_full,hvcoord)
    ...
    use control_mod,    only: nu, nu_s, hypervis_order, hypervis_subcycle, nu_p, nu_top
...
!phl                    heating = (vtens(i,j,1,k,ie)*v1  + vtens(i,j,2,k,ie)*v2 )
                 elem(ie)%state%T(i,j,k,nt)=elem(ie)%state%T(i,j,k,nt) &
                      +ttens(i,j,k,ie)!phl -heating/cpair
               enddo
             enddo
           enddo
         end do !phl
...
!  call t_adj_detailf(-1)
   ...
   subroutine compute_and_apply_rhs(np1,nm1,n0,dt2,elem,hvcoord,hybrid,&
     ...
     !
     ! ===================================
     use dimensions_mod, only : np, nc, nlev, lcp_moist
     use dimensions_mod, only : qsize_condensate_loading, ntrac
     use dimensions_mod, only : qsize_condensate_loading_idx,qsize_condensate_loading_cp
     use hybrid_mod, only : hybrid_t
     use element_mod, only : element_t
     ...
     use bndry_mod, only : bndry_exchangev
     use hybvcoord_mod, only : hvcoord_t
     use physconst, only: cpair, rair, epsilo
     ...
     type (element_t)     , intent(inout), target :: elem(:)
     type (derivative_t)  , intent(in) :: deriv
     real (kind=r8)       , intent(out):: inv_cp_full(np,np,nlev,nets:nete)
     ...
     real (kind=r8), dimension(np,np,2,nlev):: vdp_full !dry+vapor
     ...
     real (kind=r8), dimension(np,np,2,nlev) :: grad_dp_dry
     ...
     real (kind=r8) :: stashdp3d (np,np,nlev)
     real (kind=r8) :: tempdp3d  (np,np)
     real (kind=r8) :: tempflux  (nc,nc,4)
     real (kind=r8) :: inv_epsilon, ckk, term, T_v(np,np,nlev)
     
     type (EdgeDescriptor_t)                                       :: desc
     
     real (kind=r8) :: sum_water(np,np,nlev), density_inv, sum_cp
     
     real (kind=r8) ::  E,v1,v2
     real (kind=r8) ::  glnps1,glnps2
     integer :: i,j,k,kptr,ie,nq,m_cnst
     ...
     do ie=nets,nete
       ...
       do k=1,nlev
         ...
         do nq=1,qsize_condensate_loading
           ...
           !
           elem(ie)%state%q(:,:,k,m_cnst) =elem(ie)%state%Qdp(:,:,k,m_cnst,qn0)/elem(ie)%state%dp3d(:,:,k,n0)
           sum_water(:,:,k) = sum_water(:,:,k) + elem(ie)%state%q(:,:,k,m_cnst)
         end do
         do j=1,np
           do i=1,np
...
!         dp_full(:,:,k)=(1+elem(ie)%state%q(:,:,k,1))*dp_dry(:,:,k)
         ...
         call gradient_sphere(dp_dry(:,:,k),deriv,elem(ie)%Dinv,grad_dp_dry(:,:,:,k))
         ...
         do k=1,nlev
           do j=1,np
             do i=1,np
               ...
               do nq=1,qsize_condensate_loading
                 m_cnst = qsize_condensate_loading_idx(nq)
                 sum_cp = sum_cp+qsize_condensate_loading_cp(nq)*elem(ie)%state%q(i,j,k,m_cnst)
               end do
               inv_cp_full(i,j,k,ie)   = sum_water(i,j,k)/sum_cp
             end do
           end do
         end do
       else
         inv_cp_full = 1.0_r8/cpair
       end if
       ! ==============================================
        ...
        !DIR_VECTOR_ALIGNED
         ...
         do j=1,np
           do i=1,np
             ...
             !             density_dry = p_dry(i,j,k)/(Rair*elem(ie)%state%T(i,j,k,n0))
...
!$omp parallel do private(k)
       ...
       ! when debugging: also update ps
         !elem(ie)%state%psdry(:,:,np1) = elem(ie)%rspheremp(:,:)*elem(ie)%state%psdry(:,:,np1)
     end do
...
!$OMP BARRIER
   ...
   subroutine smooth_phis(phis,elem,hybrid,deriv,nets,nete,minf,numcycle)
     ...
     real (kind=r8) :: mx,mn
     integer :: ie,ic,i,j,order,order_max, iuse
...
!$OMP BARRIER
  ...
  subroutine calc_tot_energy_dynamics(elem,nets,nete,tl,tl_qdp,outfld_name_suffix)
    ...
    use physconst,              only: gravit, cpair, cpliq, cpice, cpwv
    use element_mod,            only: element_t
        ...
        do k = 1, nlev
          do j=1,np
            do i = 1, np
              ...
              if (lcp_moist) then
                pdel     = elem(ie)%state%dp3d(i,j,k,tl)*elem(ie)%state%T(i,j,k,tl)/gravit
                dp3d_inv = 1.0_r8/elem(ie)%state%dp3d(i,j,k,tl)
                se_tmp   = cpair+cpwv*elem(ie)%state%qdp(i,j,k,1,tl_qdp)*dp3d_inv 
                if (ixcldliq > 1) se_tmp=se_tmp+cpliq*elem(ie)%state%qdp(i,j,k,ixcldliq,tl_qdp)*dp3d_inv
                if (ixcldice > 1) se_tmp=se_tmp+cpice*elem(ie)%state%qdp(i,j,k,ixcldice,tl_qdp)*dp3d_inv
                se_tmp = se_tmp*pdel
              else
...
!              do j = 1, nc
!                do i = 1, nc
!                  wl_tmp   = fvm(ie)%dp_fvm(i,j,k,n_fvm)*fvm(ie)%c(i,j,k,ixcldliq,n_fvm)/gravit
...
!          end if
  ...
  subroutine output_qdp_var_dynamics(qdp,nets,nete,outfld_name)
   ...
   subroutine compute_omega(hybrid,n0,elem,deriv,nets,nete,dt,hvcoord)
     use control_mod,    only : nu, hypervis_order, hypervis_subcycle
     ...
     real (kind=r8) :: divdp_full(np,np,nlev),vdp_full(np,np,2),grad_dp_dry(np,np,2,nlev)
     ...
     do ie=nets,nete
        ...
        do nq=1,qsize_condensate_loading
           m_cnst = qsize_condensate_loading_idx(nq)
           sum_water(:,:,:) = sum_water(:,:,:) + elem(ie)%state%q(:,:,:,m_cnst)
        end do
        do k=1,nlev
         ...
         call gradient_sphere(elem(ie)%state%dp3d(:,:,k,n0),deriv,elem(ie)%Dinv,grad_dp_dry(:,:,:,k))
       end do
       ckk       = 0.5_r8
       suml(:,:  ) = 0
       do k=1,nlev
     ...
     call bndry_exchangeV(hybrid,edgeOmega)
     do ie=nets,nete
       kptr=0
     ...
     if  (del4omega) then
       ...
       if (hypervis_order == 1) then
         call endrun( 'ERROR: compute_omega not coded for hypervis_order == 1')      
         ...
         do ic=1,hypervis_subcycle
           ...
           do ie=nets,nete
             do k=1,nlev
               Otens(:,:,k,ie) = -dt_hyper*nu*Otens(:,:,k,ie)
             end do
             kptr=0
           ...
           do ie=nets,nete
             do k=1,nlev
               elem(ie)%derived%omega(:,:,k) =elem(ie)%derived%omega(:,:,k)+&
                    6.25_r8*elem(ie)%rspheremp(:,:)*Otens(:,:,k,ie)
             end do
           end do
         end do
       else
         call endrun( 'ERROR: compute_omega hypervis_order out of range')      
       end if
     end if
  ...
  subroutine calc_dp3d_reference(elem,deriv,edge3,hybrid,nets,nete,nt,hvcoord,dp3d_ref)
    ...
    !=======================================================================
    use hybvcoord_mod  ,only: hvcoord_t
    use physconst      ,only: zvir,rair,cappa
    use cam_abortutils ,only: endrun
    ...
    do ie=nets,nete
      ...
      !
      Tv_val(:,:) = rair*elem(ie)%state%T(:,:,nlev,nt)!*(1._r8+zvir*elem(ie)%state%Q(:,:,nlev,1))
